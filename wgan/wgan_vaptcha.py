...
class WGAN():
    ...
    def sample_images(self, epoch):
        noise = np.random.normal(0, 1, (9, self.latent_dim))
        gen_imgs = self.generator.predict(noise)

        # Rescale images 0 - 1
        gen_imgs = 0.5 * gen_imgs + 0.5

        plt = draw_color.draw_matrix(gen_imgs)
        plt.savefig("images/vaptcha_%d.png" % epoch)
        plt.close()


if __name__ == '__main__':
    wgan = WGAN()
    wgan.train(epochs=3000, batch_size=32, sample_interval=1000)
